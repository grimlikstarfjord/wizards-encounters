...
public class DBHandler
{
	...
	public static boolean updatePlayer(Player player)
	{
		...
		String command =
			"UPDATE Player SET Class = '" + cl + "', " + "Rank = '" + ra + "', " + "EquippedWeapon = '" + ew + "', "
	...
	public static boolean updateActiveAbilities(int[] items, int playerID)
	{
		...
		{
			command =
				"INSERT INTO ActiveAbilities (AbilityID, PlayerID) VALUES ('" + items[a] + "', '" + playerID + "')";
	...
	public static boolean updateImpactPref(Context context, boolean b)
	{
		if (!db.isOpen())
			open(context);
	...
	public static boolean updateSoundPrefs(Context context)
	{
		...
		String command = "UPDATE GlobalStats SET PlayMusic = '" + m + "', PlaySounds = '" + s + "'";
	...
	public static int insertNewPlayer(String playerName, int playerClass)
	{
		...
		String command =
			"INSERT INTO Player (Name, Class, CurrentHP, CurrentAP) VALUES ('" + playerName + "','" + playerClass
				+ "','" + curHP + "','" + curAP + "')";
		try
		{
			db.execSQL(command, new String[]
	...
	public static boolean updateOwnedItems(StoreItem[] items)
	{
		...
		for (int a = 0; a < items.length; a++)
		{
			if (a == 0)
			{
				command += " SELECT ";
				command +=
					"'" + items[a].itemType() + "' AS 'ItemType'," + "'" + items[a].id() + "' AS 'ItemID'," + "'"
			...
			else
			{
				...
				command +=
					"'" + items[a].itemType() + "'," + "'" + items[a].id() + "'," + "'" + items[a].chargesLeft() + "',"
	...
	public static void getSoundPrefs(Context context)
	{
		...
		String command = "SELECT PlayMusic, PlaySounds FROM GlobalStats";
		Cursor cursor = null;
		try
		{
			cursor = db.rawQuery(command, null);
			...
			for (int i = 0; i < cursor.getCount(); i++)
			{
				...
				Log.d("dbHandler",
					"row" + i + ": ItemType=" + cursor.getInt(0) + ", ItemId=" + cursor.getInt(1) + ", ChargesLeft="
						+ cursor.getInt(2) + ", chargesPurch=" + cursor.getInt(3) + ", amount=" + cursor.getInt(4));
	...
	{
		String command =
			"INSERT INTO GlobalStats (Gold, PlayMusic, PlaySounds, Impact) VALUES ('"
				+ DefinitionGlobal.DEFAULT_STARTING_GOLD + "', 1, 1, 1);";
	...
	{
		String command =
			"CREATE TABLE ActiveAbilities (ID INTEGER PRIMARY KEY autoincrement, "
	...
	{
		String command =
			"CREATE TABLE OwnedItems (ItemType INTEGER NOT NULL DEFAULT '-1', "
	...
	{
		String command =
			"CREATE TABLE IF NOT EXISTS GlobalStats (Gold INTEGER NOT NULL DEFAULT '100', PlayMusic INTEGER NOT NULL DEFAULT '1', PlaySounds INTEGER NOT NULL DEFAULT '1', Impact INTEGER NOT NULL DEFAULT '1');";
	...
	private static boolean openDB(Context mContext)
	{
		...
		{
			db =
				mContext.openOrCreateDatabase(mContext.getResources().getString(R.string.dbnamestring),
